...
// ---------------------------------------------------------------------------
// Fallback opt-in resolution & episode/notification gating (pure)
// ---------------------------------------------------------------------------
// Investigation finding (2026-07-30): the real limit messages this repo has to
// parse (see MUST_MATCH in test/detect.test.mjs) NEVER name a model. They only
// carry a time-window qualifier (session/usage/rate/weekly/daily/monthly/
// 5-hour/none). There is therefore NO reliable textual signal to distinguish
// "the top-tier model's own allowance is exhausted" from "a generic
// daily/weekly allowance shared across tiers is exhausted" — recordModel()
// only tells us which model happened to be active, not which kind of bar was
// hit. Inventing a keyword heuristic on top of that would be guessing, not
// detecting. Consequence: automatic model fallback must default to OFF
// (wait for the reset) and only ever run when a human explicitly opts in for
// that run, accepting the risk described in the README caveats.

/**
 * Resolve whether automatic model-fallback is active for this run.
 * Precedence: `--no-fallback` always wins (explicit, always safe) >
 * `--fallback` / `AUTO_RESUME_FALLBACK=1|on|true|yes` (explicit opt-in) >
 * default OFF.
 */
export function resolveFallbackEnabled({ noFallbackFlag = false, fallbackFlag = false, envValue = '' } = {}) {
  if (noFallbackFlag) return false;
  if (fallbackFlag) return true;
  return /^(1|on|true|yes)$/i.test(String(envValue || '').trim());
}

/**
 * Whether `episode` (persisted bookkeeping for one limit outage) is a brand
 * new outage, or the same still-open one being re-detected (watcher restart,
 * catch-up scan, or a fresh transcript after a resume that didn't clear the
 * limit). Only a new episode should reset `triedTiers` and earn a fresh
 * notification — re-detecting the same episode must stay silent.
 */
export function isNewEpisode(episode, now) {
  return !episode || now > (episode.resetAt || 0);
}

/**
 * One notification per REAL event: a brand new episode, or (only relevant
 * when fallback is explicitly enabled) an actual escalation to a new model
 * tier. A re-detection of an already-notified, still-open episode with no
 * tier escalation must not notify again (anti-spam).
 */
export function shouldNotifyLimitDetected(isNewEp, fallbackTier) {
  return Boolean(isNewEp || fallbackTier);
}
